...
export default {
    ...
    mounted(){
        storage.getItem('setPersonalMessage',  (event)=> {
            ...
            var arr1=[{title:'格式/批量状态:',content:""},
                    {title:'送往单位:',content:""},
                    {title:'申请用途:',content:""},
                    {title:'用印内容:',content:""}];
            var arr2=[{title:'会签状态:',content:""},
                    {title:'申请文件名称:',content:""},
                    {title:'文件性质:',content:""},
                    {title:'文件类型:',content:""},
                    {title:'联系人:',content:""},
                    {title:'联系电话:',content:""}];
            if(this.indexDetailObj.applyNo.slice(0,2)!='LC'){ //
                this.title='格式/批量状态';
                this.indexDetailObj.isContract='0';
                this.DetailLists=arr1;
                this.TitleText='格式/批量状态信息'
            }else{
                this.title='法规会签审批报告';
                this.TitleText='会签申请信息';
                this.indexDetailObj.isContract='1';
                this.DetailLists=arr2;
            };
            this.getInfoCheck()
            }) 
        });  
    ...
    methods:{
        getInfoCheck(){
            var getData = {};
            getData.url='mobile.showHistoryDocTable';
            getData.params = {
            applyNo:this.indexDetailObj.bsgLowaApplyPreFlowDTO.applyNo,
            isContract:this.indexDetailObj.isContract,
            }
            weexRequest.getRequestResult(getData, (res) => {
              var con = res.returnObject;
              if(res.successed){
                if(this.indexDetailObj.isContract=='0'){
                    this.DetailLists[0].content=con.approvalState;
                    this.DetailLists[1].content=con.approvalInfo.sendSection;
                    this.DetailLists[2].content=con.approvalInfo.content;
                    this.DetailLists[3].content=con.approvalInfo.remark;
                    this.opinions=con.docList;
                }else{
                    this.DetailLists[1].content=con.approvalInfo.fileName;
                    this.DetailLists[2].content=con.approvalInfo.filePropStr;
                    this.DetailLists[3].content=con.approvalInfo.fileTypeStr;
                    this.DetailLists[4].content=con.approvalInfo.contacts;
                    this.DetailLists[5].content=con.approvalInfo.contTel;
                    this.opinions=con.docList;
                    this.DetailLists[0].content=con.approvalState;
                }
              }else{
