<template>
    ...
    <div class="wrapper">
        ...
        <scroller class="detail" show-scrollbar="false">
            ...
            <div v-for="(item,index) in showFileList" :key="index">
                ...
                <div class="certificatesBlock" v-if="item.templateType=='ZZ'">
                    ...
                    <div class="wordItem wordItemTop">
                        ...
                        <div class="wordItemTitle wordItemShow" >
                            <!-- <image class="wordItemContentIcon" :src="wordIcon" />          -->
                            <text  class="wordItemContentText blueFont TitleTextFZ" @click="filePreview(1,item)">{{item.showFileName}}</text>          
            ...
            <div class="DetailCell" v-for="(list,index) in initiatorOpinionList" :key="index">               
                ...
                <div v-if="list.title=='文件名称:'">
                   <text class="contentText blueFont " @click="filePreview(2,fileDown)" v-for="(fileDown,inde) in list.content" :key="inde" :style="{'paddingTop': index===0? '48px': '40px'}">{{fileDown.fileName}}</text>
                </div>
                <text v-if="list.title!='文件名称:'" class="contentText" :style="{'paddingTop': index===0? '48px': '40px'}">{{list.content}}</text>
            </div>
            <div class="blank1"></div>
        ...
        <div :style="{height:'48px',backgroundColor:'#fff'}"></div>
        <div class="submit" @click="openOpinion" v-if="approvalBtnState=='00'">
        ...
        <div class="opinionInput" v-if="showOpinion">
            ...
            <div class="buttons buttonsGroup3" v-if="descType!='3'">
                <div class="button1" @click="agree(1)" :style="{backgroundColor:buttonResult? 'rgb(0,164,234)':'rgb(255,255,255)'}">
                    <text class="buttonContent" :style="{color: buttonResult? 'rgb(255,255,255)':'rgb(0,164,234)'}">同  意</text>
                </div>
                <div class="button1" @click="veto(1)" :style="{backgroundColor:buttonResult? 'rgb(255,255,255)':'rgb(0,164,234)'}">
    ...
    methods:{
        ...
        getOpinionList(applyNo){ //获取发起人意见
            ...
            weexRequest.getRequestResult(getData,(res)=>{
                ...
                if(res.successed){
                    ...
                    this.initiatorOpinionList[1].content=(con.regDate?con.regDate:'')+' '+(con.regTime?con.regTime:'');
        ...
        chooseButton(){
            modal.confirm({
                ...
                if(result==='确认'){
                    this.submitApprovalOpinion();
                    
                }
            })
            
        },
        submitApprovalOpinion(){//审批提交
            var getData = {};
            getData.url='mobile.submitApprovalOpinion';
            getData.params = {
            applyNo:this.applyNo,
            regOper:this.operNo, 
            approvalOpinion:this.buttonResult?'0':'1'
            }
            weexRequest.getRequestResult(getData, (res) => {
